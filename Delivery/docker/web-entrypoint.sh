...
if [ -n "${API_BASE_URL:-}" ] && [ -f "$APP_SETTINGS" ]; then
    ...
    # Use sed to replace the "BaseUrl" value in appsettings.json
    sed -i "s|\"BaseUrl\"[[:space:]]*:[[:space:]]*\"[^\"]*\"|\"BaseUrl\": \"${ESCAPED_URL}\"|g" "$APP_SETTINGS"
    
    echo "[Entrypoint] Successfully updated appsettings.json"
else
    echo "[Entrypoint] No API_BASE_URL provided or appsettings.json not found. Using defaults."
fi
